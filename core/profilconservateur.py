...
weights_conservateur = {
    ...
    "Dividendes": 0.25,      
    "Ratio Cours/Bénéfice (P/E)": 0.20,  
    "Volume de Transactions": 0.05,     
    "Ratio Cours/Valeur Comptable (P/B)": 0.10,  
    "Croissance des Revenus": 0.05,      
    "Marge Bénéficiaire": 0.10,          
    "Beta du Secteur": 0.20              
}
...
def normalize_data_moderate(stock_data):
    ...
    dividend_yield = stock_data["Dividendes"]
    normalized_data["Dividendes"] = dividend_yield * 7.5

    pe_ratio = stock_data["Ratio Cours/Bénéfice (P/E)"]
    normalized_data["Ratio Cours/Bénéfice (P/E)"] = (1 / (pe_ratio + 0.01)) * 7.5 if pe_ratio else 0
    ...
    pb_ratio = stock_data["Ratio Cours/Valeur Comptable (P/B)"]
    normalized_data["Ratio Cours/Valeur Comptable (P/B)"] = (1 / (pb_ratio + 0.01)) * 7.5 if pb_ratio else 0

    revenue_growth = stock_data["Croissance des Revenus"]
    normalized_data["Croissance des Revenus"] = min(10, revenue_growth * 10)

    profit_margin = stock_data["Marge Bénéficiaire"]
    normalized_data["Marge Bénéficiaire"] = profit_margin * 7.5

    beta = stock_data["Beta du Secteur"]
    normalized_data["Beta du Secteur"] = 5 * (2 - beta) if beta else 10
...
weights_moderate = {
    "Prix des Actions": 0.10,  
    "Dividendes": 0.15,      
    "Ratio Cours/Bénéfice (P/E)": 0.15,  
    "Volume de Transactions": 0.05,     
    "Ratio Cours/Valeur Comptable (P/B)": 0.15,  
    "Croissance des Revenus": 0.20,      
    "Marge Bénéficiaire": 0.10,          
    "Beta du Secteur": 0.10              
}

def fetch_and_score_stocks_moderate(tickers):
    results = {}
    for ticker in tickers:
        stock_data = get_stock_data(ticker)
        normalized_data = normalize_data_moderate(stock_data)
        score = calculate_score(normalized_data, weights_moderate)
        results[ticker] = {"Score": score, "Data": stock_data}
...
#endustriel
#tickers_example = ["AIR", "SAF.PA", "GE"]
sorted_scores_moderate = fetch_and_score_stocks_moderate(tickers_example)

# Affichage des résultats
for rank, (ticker, data) in enumerate(sorted_scores_moderate.items(), start=1):
    print(f"Classement {rank}: Ticker: {ticker}, Score: {data['Score']}")
    for key, value in data["Data"].items():
        print(f"{key}: {value}")
    print("\n")
